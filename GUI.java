...
public class GUI extends JFrame
{
    ...
    private void initComponents() {
        ...
        Calci.setFont(new java.awt.Font("Arial", 1, 12));
        Calci.setText("I");
        Calci.setPreferredSize(new java.awt.Dimension(42, 10));
                    ...
                    .addGroup(CalculatorPanelLayout.createSequentialGroup()
                        .addGroup(CalculatorPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                            ...
                            .addGroup(javax.swing.GroupLayout.Alignment.LEADING, CalculatorPanelLayout.createSequentialGroup()
                                ...
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(Calci, javax.swing.GroupLayout.PREFERRED_SIZE, 50, javax.swing.GroupLayout.PREFERRED_SIZE)))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                ...
                .addGroup(CalculatorPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                    .addGroup(CalculatorPanelLayout.createSequentialGroup()
                        ...
                        .addGroup(CalculatorPanelLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            ...
                            .addComponent(Calci, javax.swing.GroupLayout.PREFERRED_SIZE, 32, javax.swing.GroupLayout.PREFERRED_SIZE)))
    ...
    private void XActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_XActionPerformed
        ...
        //checkExpression();
    	String someExpression = getExpression();
    	//System.out.println(someExpression);
    	System.out.println(integral(someExpression));
    	drawFTCTable();
    ...
    private String integral(String someExpression){
    	//find derivative of function and use trapezoidal sum to find area under curve
    	//check if derivative is continuous; if it isn't, then FTC doesn't apply
    	...
    	if((expression.solve(someExpression,a).equals("Error") ||
    			(expression.solve(someExpression,b).equals("Error")))){
    		return "= The Fundamental Theorem of Calculus does not apply.*";
    	}
    	
    
    	double trapSum = 0;
    	for (double i=a; i<=b-0.002; i+=0.001){
    		if(/*expression.solve(someExpression,i-0.001).equals("Infinity")||*/
    				expression.solve(someExpression, i).equals("Infinity")||
    				expression.solve(someExpression, i+0.001).equals("Infinity")||
    				expression.solve(someExpression, i+0.002).equals("Infinity")){
    			 return "FTC does not apply.";
    		}
    		String firstDerivA = expression.solve((expression.solve(someExpression,i) + 
    				"-" + expression.solve(someExpression, i+0.001))+"/"+(i - (i+0.001)), i);
    		String firstDerivB = expression.solve((expression.solve(someExpression,i+0.001) + 
    				"-" + expression.solve(someExpression, i+0.002))+"/"+(i+0.001 - (i+0.002)), i);
    		double fDA = Double.parseDouble(firstDerivA), fDB = Double.parseDouble(firstDerivB);
    		trapSum += 0.001/2*(fDA+fDB);	
    	}
    	
    	
    	
    	String functionA = expression.solve(someExpression, a);
    	String functionB = expression.solve(someExpression, b);
    	double fA = Double.parseDouble(functionA);
    	double fB = Double.parseDouble(functionB);
    	double integral = fB - fA;
    	
    	if(trapSum==integral){
    		System.out.println("FTC applies.");
    	}
    	
    	return "F(Xmax) - F(Xmin) = " + integral + "\n" +
    			"*Integral: " + trapSum;
    }
    ...
    private void drawFTCTable()
    {
       //end=Double.parseDouble(End.getText());
      // start=Double.parseDouble(Start.getText());
      // increment=Double.parseDouble(Increment.getText());
       //rows=(int)Math.round(((end-start)/increment)+.5);        
        String[][] g=new String [1][2];int i=0;
        
        String someExpression = getExpression();
        String integrals = integral(someExpression);
        String FTC = integrals.substring(integrals.indexOf("= ")+2, integrals.indexOf("*"));
        String integralVal = integrals.substring(integrals.indexOf(": ")+2, integrals.length());
        g[0][0] = FTC;
        g[0][1] = integralVal;
        /*for (double d=start;i<rows;d+=increment)
        {
            g[1][0]=Double.toString(d);
            i++;
        }
        for (i=0;i<rows;i++)
        {
        ...
        table.setModel(new javax.swing.table.DefaultTableModel
            (g,new String []{"f(Xmax) - f(Xmin)", "Integral of f'(x)"}));
    }
